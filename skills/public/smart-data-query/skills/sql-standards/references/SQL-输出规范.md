...
- Hive（尤其低版本）兼容性：
  - `ORDER BY` 必须只引用“当前 SELECT 的输出列名”，避免报 `Invalid table alias or column reference`。
  - 不要使用 `ORDER BY 1/2/3` 这类序号排序（兼容性差，且你们环境明确不允许）。
- 输出字段给业务验收：最终 `SELECT` 建议使用中文别名；Hive/SparkSQL 下推荐 `as \`中文列名\``，GaussDB 下推荐 `as \"中文列名\"`。
