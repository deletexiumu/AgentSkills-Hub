...
素材收集完成后，输出覆盖情况表（章节 × 素材来源），然后**自动进入阶段 4**。
...
> 仅在 `config.yaml` 的 `review.models` 配置了评审模型时执行。未配置则跳过此阶段，直接进入阶段 5。
> **本阶段自动执行。** 素材收集完成后直接发起评审。
...
- 汇总评审发现（表格 + 严重度 + 采纳状态），向用户展示后**自动进入阶段 5**
...
> **本阶段自动执行。** 标题定稿后直接发起外部评审，评审通过后自动进入阶段 7。
